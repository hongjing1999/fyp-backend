...
public class DroneUserDroneService {
    ...
    public DroneUserDroneService(
        ...
        DroneUserRepository droneUserRepository,
    ...
    private DroneTelemetryGraphDTO getDateList(
        ...
        while (startInstant.compareTo(stopInstant) <= 0) {
            ...
            boolean removed = droneTelemetryDTOList.removeIf(
                e -> {
            ...
            startInstant =
                ZonedDateTime.ofInstant(startInstant, ZoneId.of("Asia/Kuala_Lumpur")).plus(1, unit).truncatedTo(unit).toInstant();
            if (!removed) {
    ...
    private List<DroneTelemetryDTO> getAverage(List<DroneTelemetryDTO> droneTelemetryDTOList, TemporalUnit unit) {
        ...
        return droneTelemetryDTOList
            .stream()
            .collect(Collectors.groupingBy(arr -> roundUp(arr.getCreatedDate(), unit)))
            .entrySet()
            .stream()
            .map(
                e -> {
